...
// promisify
const writeFile$1 = util__default['default'].promisify(writeFileFn__default['default']);
...
async function getInfo() {
  ...
  for (const item of dir) {
    ...
    try {
      const jsonData = JSON.parse(await readFile$2(path__default['default'].join(cachePath, item), { encoding: 'utf-8' }));
...
describe('signature', async () => {
  it('should return an object', async () => {
    ...
    assert__default['default'].equal(typeof info, 'object');
    assert__default['default'].equal(util.isArray(info.cacheItems), true);
    assert__default['default'].equal(typeof info.signature, 'string');
    assert__default['default'].equal(typeof info.cachePath, 'string');
  });
});
