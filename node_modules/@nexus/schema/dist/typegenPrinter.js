        ...
        utils_1.eachObj(rootTypings, (rootType, typeName) => {
            if (typeof rootType !== 'string') {
                ...
                if (typeof rootTypePath !== 'string' || !path.isAbsolute(rootTypePath)) {
                    throw new Error(`Expected an absolute path for the root typing path of the type ${typeName}, saw ${rootTypePath}`);
                }
                if (!fs.existsSync(rootTypePath)) {
                    throw new Error(`Root typing path ${rootTypePath} of the type ${typeName} does not exist`);
                }
                const importPath = utils_1.relativePathTo(rootType.path, outputPath)
                    .replace(/(\.d)?\.ts/, '')
                    .replace(/\\+/g, '/');
